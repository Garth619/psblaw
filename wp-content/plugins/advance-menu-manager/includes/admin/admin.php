<?php
	...
	{
		include('include/popup_pdd_menu_content.php');
	}
            ...
            <p id="menu-item-url-wrap">
                <label class="howto" for="custom-menu-item-url">
                    ...
                    <input id="custom-menu-item-url"
                           name="menu-item[<?php echo esc_attr($_nav_menu_placeholder); ?>][menu-item-url]" type="text"
            ...
            <p id="menu-item-name-wrap">
                <label class="howto" for="custom-menu-item-name">
                    ...
                    <input id="custom-menu-item-name"
                           ...
                           type="text" class="regular-text menu-item-textbox input-with-default-title"
        ...
        <div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv amm_item_main_wrapper">
            ...
            <div class="add_mew_item_wrapper amm_deactive">
                <div class="add_item_inner_main">
                    ...
                    <div class="add_item_submit_row_wrapper">
                        ...
                        <b>Want to add a <?php echo esc_html($post_type_name); ?>? Buy the Pro Version to access all the
                            features.</b>
                        <br><br><br>
                        <button type="button" class="button-secondary amm_menu_add_cancel">Cancel</button> &nbsp;&nbsp;&nbsp;
                        <a href="https://www.thedotstore.com/advance-menu-manager-wordpress/" target="_blank"
                           class="button-primary">Buy Pro Version</a>
                        <br><br><br>
                    </div>
                </div>
            ...
            <div class="amm_header_main">
				...
				<span class="list-controls">
					<a href="<?php echo esc_url(add_query_arg(array($post_type_name . '-tab' => 'all', 'selectall' => 1,), remove_query_arg($removed_args))); ?>#posttype-<?php echo $post_type_name; ?>"
                       class="select-all"><?php _e('Select All'); ?></a>
				</span>
                    <span class="allready-menu-item"><input type="checkbox" name='curent-menu_item'
                                                            class="curent-menu_item" value=""
                                                            data-selector="<?php echo esc_attr($post_type_name) . "checklist"; ?>"/>Hide existing menu items </span>
					<?php
					...
					?>
                    <span class="page-title-action"><?php echo esc_attr($newpage_lable); ?></span>
                </div>
                <div class="menu_item_search_wrapper"><input type="text" class="menu_item_search" value=""
                                                             placeholder="Search menu item.."/></div>
            </div>

            <div id="<?php echo esc_attr($post_type_name); ?>-all"
                 class="tabs-panel tabs-panel-view-all <?php echo('all' == $current_tab ? 'tabs-panel-active' : 'tabs-panel-inactive'); ?> md_popup_main_wrapper">
                ...
                <div class="amm_menu_item_main_content_wrapper">
					...
					?>
                    <div class="menu_item_filter_header amm_popup_header_wrapper <?php echo esc_attr($post_data_calss); ?>"
                         amm-filter-selector="<?php echo esc_attr($post_type_name); ?>checklist">
						...
						<?php
							...
							if (count($filter_author) > 1) {
								...
								foreach ($filter_author as $author_id) echo '<option value="' . esc_attr($author_id) . '">' . get_the_author_meta('display_name', $author_id) . '</option>';
								echo '</select>';
							} else {
							...
							if ('page' == $post_type_name) {
								...
								if (count($filter_template) > 1) {
									...
									foreach ($templates_all as $template_name => $display_template_name) echo '<option class="' . esc_attr($template_name) . '" value="' . esc_attr($display_template_name) . '">' . esc_html($template_name) . '</option>';
									echo '</select>';
								} else {
								...
								if (count($filter_cate) >= 1) {
									...
									foreach ($filter_cate as $filter_catedata) echo '<option class="' . esc_attr($filter_catedata) . '" value="' . esc_attr($filter_catedata) . '">' . esc_html($filter_catedata) . '</option>';
									echo '</select>';
								} else {
                    ...
                    <ul amm_menu_query='page' amm_page_count="<?php echo esc_attr($total_page_count); ?>"
                        amm_post_type="<?php echo esc_attr($post_type_name); ?>"
                        id="<?php echo esc_attr($post_type_name); ?>checklist"
                        data-wp-lists="list:<?php echo esc_attr($post_type_name); ?>"
                        class="categorychecklist form-no-clear amm_popup_header <?php echo esc_attr($filter_class_name) . ' ' . esc_attr($post_data_calss); ?>"
                        amm_post_per_page='<?php echo esc_attr($post_per_page); ?>'>
						<?php
							echo $checkbox_items;
						?>
                    </ul>
			...
			<span class="add-to-menu">
                ...
                <span class="add-menu-item-pagelinks"
                      amm-pagination="<?php echo esc_attr($post_type_name); ?>checklist"></span>
            </p>
            <p class="amm_list_of_page">
				<?php
					
					$list_of_page_no = get_option('amm_post_perpage_option');
					if (empty($list_of_page_no)) $list_of_page_no = '50,100,200,500,700,1000';
					
					$amm_post_perpage_array = explode(',', $list_of_page_no);
					if (count($amm_post_perpage_array) > 0) {
						?>
                        <label>Show items per page</label>
                        <select name="amm_post_perpage_w" class="amm_post_perpage"
                                data_post_per_page="amm_<?php echo esc_attr($post_type_name); ?>"
                                data_pagination="<?php echo esc_attr($post_type_name); ?>checklist">
	...
	function wp_nav_menu_item_taxonomy_meta_box_own_md($object, $taxonomy)
	{
		...
		?>
        <div id="taxonomy-<?php echo esc_attr($taxonomy_name); ?>" class="taxonomydiv amm_item_main_wrapper">
            <div id="tabs-panel-<?php echo esc_attr($taxonomy_name); ?>-all"
                 class="tabs-panel tabs-panel-view-all tabs-panel-active md_popup_main_wrapper">
                <div class="amm_header_main">
                    <div class="new_item_add_wrapper">
                        <span class="list-controls amm_taxonomy"> <a
                                    href="<?php echo esc_url(add_query_arg(array($taxonomy_name . '-tab' => 'all', 'selectall' => 1,), remove_query_arg($removed_args))); ?>#taxonomy-<?php echo esc_attr($taxonomy_name); ?>"
                                    class="select-all"><?php _e('Select All'); ?></a></span>
                    </div>
                </div>

                <div class="menu_item_filter_header amm_popup_header_wrapper taxonomy_item_list"
                     amm-filter-selector="<?php echo esc_attr($taxonomy_name); ?>checklist">
                    <span class="title md_walker" amm-filter-title="all"><strong>Title</strong></span>
                ...
                <ul amm_menu_query='taxonomy' amm_page_count="<?php echo esc_attr($total_page_count); ?>"
                    amm_post_type="<?php echo esc_attr($post_type_name); ?>"
                    id="<?php echo esc_attr($taxonomy_name); ?>checklist"
                    data-wp-lists="list:<?php echo esc_attr($taxonomy_name) ?>"
                    class="categorychecklist form-no-clear amm_popup_header taxonomy_item_list"
                    amm_post_per_page='<?php echo esc_attr($post_per_page); ?>'>
					<?php
						$args['walker'] = $walker;
			...
			<span class="add-to-menu">
                ...
                <span class="add-menu-item-pagelinks"
                      amm-pagination="<?php echo esc_attr($taxonomy_name); ?>checklist"></span>
            </p>
            <p class="amm_list_of_page">
				<?php
					...
					if (count($amm_post_perpage_array) > 0) {
                        ...
                        <label>Show items per page</label>
                        <select name="amm_post_perpage_w" class="amm_post_perpage"
                                data_post_per_page="amm_<?php echo esc_attr($taxonomy_name); ?>"
                                data_pagination="<?php echo esc_attr($taxonomy_name); ?>checklist"">
						<?php foreach ($amm_post_perpage_array as $data) {
							if ($post_per_page == $data) {
								echo '<option value="' . esc_attr($data) . '" selected>' . esc_html($data) . '</option>';
							} else {
