...
class md_admin_interface {
    ...
    function amm_save_existing_menu($menu_items) {
	    ...
	    check_ajax_referer( 'amm-ajax-nonce', 'amm-ajax-nonce', false );
        ...
        $current_edit_menu_id = sanitize_text_field( $_REQUEST['current_edit_menu_id'] );
        ...
        if (isset($_POST['menu-locations'])) {
            $new_menu_locations = array_map('absint', wp_unslash( $_POST['menu-locations'] ) );
        ...
        $menu_title = trim(sanitize_text_field($_POST['menu-name']));
        if (!$menu_title) {
        ...
        if (!is_wp_error($_menu_object)) {
            $old_menu_name = sanitize_text_field(( $_POST['old-menu-name'] ) );
        ...
        if (!empty($_REQUEST['menu-item-db-id']))
            $menu_item_db_id = array_values(array_map( 'absint', wp_unslash( $_REQUEST['menu-item-db-id'] ) ) );
        $menu_item_db_id_count = is_array($menu_item_db_id) ? count($menu_item_db_id) : '0';
        $end = (1 + $menu_item_db_id_count) - 1;
        $keys = range(1, $end);

        $deleted_nodes = explode(',', sanitize_text_field( $_REQUEST['delete_menu_items']));
    ...
    function my_action_ajax_for_create_menu() {
        ...
        $menu_name = sanitize_text_field( $_POST['new_menu_name'] );
    ...
    function menu_container_print_custom() {
        ...
        if (isset($_GET['action']) && 'locations' == $_GET['action']) {
            $menu_location_flag = sanitize_text_field( $_GET['action'] );
        } else {
            $menu_location_flag = '';
        }
        ...
        wp_enqueue_script('fancy-alert', esc_url( PLUGIN_PATH . 'js/fancy_alert.js' ), array('jquery'), false);
        wp_enqueue_script('md-pagination', esc_url( PLUGIN_PATH . 'js/md_pagination.js' ), array('jquery'), false);
        ?>
            ...
            <div style="overflow: hidden;" id="wpbody-content" aria-label="Main content" tabindex="0">
                <div class="wrap">
                    ...
                    <?php else : ?>
                        ...
                        <div id="nav-menus-frame" class="menu_manager_plus" <?php echo esc_attr( $display_none_property ); ?>>
                            <div id="menu-management">
                                <form action="" method="post" enctype="multipart/form-data" id="md_amm_menu_form">
                                    ...
                                    <div id="nav-menu-header">
                                        <div class="major-publishing-actions">
                                            <label class="menu-name-label howto open-label" for="menu-name">
                                                <span>Menu Name</span>
                                                <input name="menu-name" id="menu-name" type="text" class="menu-name regular-text menu-item-textbox" title="Enter menu name here" value="<?php if (!empty($current_selected_menu_name)) echo esc_attr( $current_selected_menu_name ); ?>">
                                    ...
                                    <?php
                                    $menu_items = wp_get_nav_menu_items($nav_menu_selected_id);// WPCS: XSS OK.
                                    $data = array();
                                    $mydata = $menu_items;
                                    ?>
                                                                        ...
                                                                        <div class="menu-item-bar">
                                                                            <div class="menu-item-handle ui-sortable-handle">
                                                                                ...
                                                                                <span class="item-controls"> 
                                                                                    ...
                                                                                    <span class="menu_item_type"><?php echo esc_html( $menu_items[$amm]->type_label ); ?></span>
                                                                                    <span class="menu_sub_details" title="View Attributes">&nbsp;</span>
                                                                                    <span data-attr-menu-item='<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>' class="delete_node" title="Delete this item">X</span>
                                                                                </span>
                                                                            </div>
                                                                            <span class="my-menu-controls">
                                                                                <span class="my-menu-controls-groups">
                                                                                    ...
                                                                                    ?>
                                                                                    <span id="" class="click block_hide_show <?php echo esc_attr( $menu_exisit_class_name ); ?>" title="Hide/Show child menu item"></span>
                                                                        ...
                                                                        <div id="menu-item-settings-<?php echo $menu_items[$amm]->db_id; ?>" class="menu-item-settings menu-manager-plus-setting">
                                                                            <?php if ('custom' == $current_menu_item_type) : ?>
                                                                                <p class="field-url description description-wide">
                                                                                    <label for="edit-menu-item-url-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                        <?php _e('URL'); ?><br />
                                                                                        <input type="text" id="edit-menu-item-url-<?php echo esc_attr( $current_menu_item_id ); ?>" class="widefat code edit-menu-item-url" name="menu-item-url[<?php echo esc_attr( $current_menu_item_id ); ?>]" value="<?php echo esc_attr($current_menu_item_url); ?>" />
                                                                                    </label>
                                                                                </p>
                                                                            <?php endif; ?>
                                                                            <p class="description description-wide">
                                                                                <label for="edit-menu-item-title-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                    <?php _e('Navigation Label'); ?><br />
                                                                                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr( $current_menu_item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $current_menu_item_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->title); ?>" />
                                                                                </label>
                                                                            </p>
                                                                            <p class="field-title-attribute description description-wide">
                                                                                <label for="edit-menu-item-attr-title-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                    <?php _e('Title Attribute'); ?><br />
                                                                                    <input type="text" id="edit-menu-item-attr-title-<?php echo esc_attr( $current_menu_item_id ); ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo esc_attr( $current_menu_item_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->post_excerpt); ?>" />
                                                                                </label>
                                                                            </p>
                                                                            <p class="field-link-target description">
                                                                                <label for="edit-menu-item-target-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                    <input type="checkbox" id="edit-menu-item-target-<?php echo esc_attr( $current_menu_item_id ); ?>" value="_blank" name="menu-item-target[<?php echo esc_attr( $current_menu_item_id ); ?>]"<?php checked($menu_items[$amm]->target, '_blank'); ?> />
                                                                                    <?php _e('Open link in a new window/tab'); ?>
                                                                                </label>
                                                                            </p>
                                                                            <p class="field-css-classes description description-thin">
                                                                                <label for="edit-menu-item-classes-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                    <?php _e('CSS Classes (optional)'); ?><br />
                                                                                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $current_menu_item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $current_menu_item_id ); ?>]" value="<?php echo esc_attr(implode(' ', $menu_items[$amm]->classes)); ?>" />
                                                                                </label>
                                                                            </p>
                                                                            <p class="field-xfn description description-thin">
                                                                                <label for="edit-menu-item-xfn-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                    <?php _e('Link Relationship (XFN)'); ?><br />
                                                                                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $current_menu_item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $current_menu_item_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->xfn); ?>" />
                                                                                </label>
                                                                            </p>
                                                                            <p class="field-description description description-wide">
                                                                                <label for="edit-menu-item-description-<?php echo esc_attr( $current_menu_item_id ); ?>">
                                                                                    <?php _e('Description'); ?><br />
                                                                                    <textarea id="edit-menu-item-description-<?php echo esc_attr( $current_menu_item_id ); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr( $current_menu_item_id ); ?>]"><?php echo esc_html($menu_items[$amm]->description); // textarea_escaped      ?></textarea>
                                                                            ...
                                                                            <input class="menu-item-data-db-id" name="menu-item-db-id[<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->db_id); ?>" type="hidden">
                                                                            <input class="menu-item-data-object-id" name="menu-item-object-id[<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->object_id); ?>" type="hidden">

                                                                            <input class="menu-item-data-object" name="menu-item-object[<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->object); ?>" type="hidden">
                                                                            <input class="menu-item-data-parent-id" name="menu-item-parent-id[<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->menu_item_parent); ?>" type="hidden">
                                                                            <input class="menu-item-data-position" name="menu-item-position[<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->menu_order); ?>" type="hidden">
                                                                            <input class="menu-item-data-type" name="menu-item-type[<?php echo esc_attr( $menu_items[$amm]->db_id ); ?>]" value="<?php echo esc_attr($menu_items[$amm]->type); ?>" type="hidden">
                                                                        </div>
