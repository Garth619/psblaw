<?php
	 ...
	 */
	class Walker_Nav_Menu_Checklist_md extends Walker_Nav_Menu
	{
		/**
		 *
		 * @param array $fields
		...
		public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
		{
			...
			$output .= '<input type="hidden" class="menu-item-db-id" name="menu-item[' . esc_attr(esc_attr($possible_object_id)) . '][menu-item-db-id]" value="' . esc_attr($possible_db_id) . '" />';
			$output .= '<input type="hidden" class="menu-item-object" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-object]" value="' . esc_attr($item->object) . '" />';
			$output .= '<input type="hidden" class="menu-item-parent-id" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-parent-id]" value="' . esc_attr($item->menu_item_parent) . '" />';
			$output .= '<input type="hidden" class="menu-item-type" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-type]" value="' . esc_attr($item->type) . '" />';
			$output .= '<input type="hidden" class="menu-item-title" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-title]" value="' . esc_attr($item->title) . '" />';
			$output .= '<input type="hidden" class="menu-item-url" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-url]" value="' . esc_attr($item->url) . '" />';
			$output .= '<input type="hidden" class="menu-item-target" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-target]" value="' . esc_attr($item->target) . '" />';
			$output .= '<input type="hidden" class="menu-item-attr_title" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-attr_title]" value="' . esc_attr($item->attr_title) . '" />';
			$output .= '<input type="hidden" class="menu-item-classes" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-classes]" value="' . esc_attr(implode(' ', $item->classes)) . '" />';
			$output .= '<input type="hidden" class="menu-item-xfn" name="menu-item[' . esc_attr($possible_object_id) . '][menu-item-xfn]" value="' . esc_attr($item->xfn) . '" />';
			if (empty($item->term_id)) {
				$output .= $item_ID;
			}
			$output .= '</span>';
			$output .= '<span class="title md_walker" title="' . esc_html($item->title) . '"> <strong>';
			...
			if (empty($item->term_id)) {
				if (!empty($item->front_or_home)) {
					...
					if (isset($item->post_name) && !empty($item->post_name)) {
						$output .= '<span class="item_slug md_walker" title="' . esc_attr($item->post_name) . '">';
					...
					if (!empty($post_author_name)) {
						...
						$output .= '<span class="author md_walker"> Admin </span>';
					}
					...
					if ('page' == $item->post_type) {
						...
						$output .= '<span class="template-list md_walker" title="' . esc_attr($display_template_name) . '">';
					...
					$output .= '<span class="publish_date md_walker">' . esc_html(get_the_date('', $item->object_id)) . '</span>';
				}
			} else {
				// display taxonomy item
				$output .= '<span class="taxonomy_slug md_walker">' . esc_html($item->slug) . '</span>';
				if (!empty($item->description)) {
					$output .= '<span class="taxomomy_content md_walker">' . esc_html($item->description) . '</span>';
				} else {
